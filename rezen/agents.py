...
class Country(Enum):
    """Country enumeration for agent searches."""
...
class AgentsClient(BaseClient):
    ...
    def get_down_line_agents(
        ...
        if status_in:
            params["statusIn"] = [s.value if isinstance(s, AgentStatus) else s for s in status_in]
    ...
    def search_payment_settings(
        ...
        if page_size is not None:
            params["pageSize"] = page_size
        if sort_direction:
            params["sortDirection"] = sort_direction.value if isinstance(sort_direction, AgentSortDirection) else sort_direction
    ...
    def search_active_agents(
        ...
        if sort_direction:
            params["sortDirection"] = sort_direction.value if isinstance(sort_direction, AgentSortDirection) else sort_direction
        if sort_by:
            params["sortBy"] = [s.value if isinstance(s, AgentSortField) else s for s in sort_by]
        if name:
            params["name"] = name
        if non_reportable:
            params["nonReportable"] = non_reportable
        if country:
            params["country"] = [c.value if isinstance(c, Country) else c for c in country]
        if state_or_province:
            params["stateOrProvince"] = [s.value if isinstance(s, StateOrProvince) else s for s in state_or_province]
