...
def generate_document() -> BytesIO:
    ...
    if HAS_REPORTLAB:
        ...
        story.append(
            Paragraph(
                "This document was uploaded via the ReZEN Python API.", styles["Normal"]
            )
        )
...
def main() -> None:
    ...
    transaction_id = (
        "cee51eff-a86f-4a62-a6dc-5de7a671c16e"  # Transaction with checklist
    )
    ...
    try:
        ...
        if not checklist_id:
            ...
            if transaction.get("checklists") and len(transaction["checklists"]) > 0:
                ...
                print(
                    "You need to create a checklist first before uploading documents."
                )
                return
        ...
        for item in items:
            if item.get("type") == "DOCUMENT" and item.get("status") not in [
                "COMPLETED",
                "COMPLETE",
            ]:
                selected_item = item
        ...
        try:
            client.checklist.complete_checklist_item(
                checklist_item_id=selected_item["id"], is_complete=True
            )
